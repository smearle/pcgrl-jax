...
from envs.reps.narrow_representation import NarrowRepresentation, NarrowRepresentationState
...
def gen_init_map(rng, tile_enum, map_shape, tile_probs):
    init_map = jax.random.choice(rng, len(tile_enum), shape=tuple(map_shape), p=tile_probs)
    return init_map

    
def gen_static_tiles(rng, static_tile_prob, n_freezies, map_shape):
    static_rng, rng = jax.random.split(rng)
    static_tiles = jax.random.bernoulli(static_rng, p=static_tile_prob, shape=map_shape)
    if n_freezies > 0:
        freezie_xys = jax.random.randint(rng, shape=(n_freezies, 2), minval=0, maxval=map_shape)
        freezie_dirs = jax.random.randint(rng, shape=(n_freezies,), minval=0, maxval=2)
        freezie_lens_empty = jnp.ones((n_freezies, 2), dtype=jnp.int32)
        freezie_lens = jax.random.randint(rng, shape=(n_freezies,), minval=0, maxval=max(map_shape))
        freezie_lens = freezie_lens_empty.at[jnp.arange(n_freezies), freezie_dirs].set(freezie_lens)
...
class PCGRLEnv(Environment):
    def __init__(self, problem: str, representation: str, map_shape: Tuple[int, int], rf_shape: Tuple[int, int], 
                 static_tile_prob, n_freezies):
        ...
        self.prob: Problem
        if problem == 'binary':
            self.prob = BinaryProblem(map_shape=map_shape)
        else:
        ...
        env_map = gen_init_map(rng, self.tile_enum, map_shape, self.tile_probs)

        self.rep: Representation
        if representation == 'narrow':
            self.rep = NarrowRepresentation(env_map=env_map, rf_shape=rf_shape, tile_enum=self.tile_enum)
        else:
    ...
    def render(self, env_state: EnvState):
        ...
        path_coords = get_path_coords(env_state.prob_state.flood_path_state.flood_count, self.prob.max_path_len)
    ...
    def action_space(self, env_params: EnvParams) -> int:
        return self.rep.action_space()

    def observation_space(self, env_params: EnvParams) -> int:
        return self.rep.observation_space()


tile_size = np.int8(16)
def render_map(env: PCGRLEnv, env_state: EnvState, path_coords: chex.Array):
    ...
    lvl_img = jnp.zeros((full_height*tile_size, full_width*tile_size, 4), dtype=jnp.uint8)
    ...
    for y in range(len(env_map)):
        for x in range(len(env_map[y])):
            ...
            lvl_img = lvl_img.at[y*tile_size: (y+1)*tile_size, x*tile_size: (x+1)*tile_size, :].set(tile_img)
    ...
    if hasattr(env_state.rep_state, 'pos'):
        ...
        lvl_img = jax.lax.dynamic_update_slice(lvl_img, x_border, (y, x+tile_size-2, 0))
        lvl_img = jax.lax.dynamic_update_slice(lvl_img, y_border, (y, x, 0))
        lvl_img = jax.lax.dynamic_update_slice(lvl_img, y_border, (y+tile_size-2, x, 0))
    ...
    if env.static_tile_prob > 0 or env.n_freezies > 0:
        ...
        i = 0
        _, lvl_img, _ = jax.lax.while_loop(cond, draw_static_tile, (static_coords, lvl_img, i))
