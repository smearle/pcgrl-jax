...
def gtk_main(config):
    ...
    env.init_graphics()
    n_metrics = len(env.prob.metric_names)

    def _init_gui(env: PCGRLEnv):
        screen_width = 200
        screen_height = 100 * n_metrics
        from gtk_gui import GtkGUI

        if not hasattr(self.unwrapped._prob, "_graphics"):
            _graphics = {}
            _graphics = env.prob.graphics
        win = GtkGUI(env=env, tile_types=tile_types, tile_images=_graphics, 
